...
module BackgroundQueue
  ...
  class Client
    ...
    def add_task(worker, owner_id, job_id, task_id, task_parameters={}, options={} )
      send_command(BackgroundQueue::Command.add_task_command(worker, owner_id, job_id, task_id, task_parameters, options ))
    end
    ...
    def add_tasks(worker, owner_id, job_id, tasks, shared_parameters={}, options={} )
      send_command(BackgroundQueue::Command.add_tasks_command(worker, owner_id, job_id, tasks, shared_parameters, options ))
    end
    
    #remove tasks from the background queue
    def remove_tasks(tasks, options={})
      send_command(BackgroundQueue::Command.remove_tasks_command(tasks, options))
    end
    ...
    def send_command_to_server(command, server)
      connection = BackgroundQueue::Connection.new(self, server)
