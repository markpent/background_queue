...
describe "Client" do
  ...
  context "initialized" do
    subject { 
      BackgroundQueue::Config.stub(:load_file).with(:path_that_exists) { 
        BackgroundQueue::Config.new(:primary_server, [:failover_1, :failover_2], :memcache_server) 
      }
    ...
    context "sending a command" do
      it "can send a command to a server" do
        BackgroundQueue::Connection.any_instance.should_receive(:send_command).with(:command) { true }
      ...
      it "can use failover server if main server fails" do
        subject.should_receive(:send_command_to_server).with(:command, :primary_server) { raise BackgroundQueue::ConnectionError, "Primary" }
      ...
      it "can use second failover server if needed" do
        subject.should_receive(:send_command_to_server).with(:command, :primary_server) { raise BackgroundQueue::ConnectionError, "Primary" }
        subject.should_receive(:send_command_to_server).with(:command, :failover_1) { raise BackgroundQueue::ConnectionError, "Failure 1" }
      ...
      it "throws an exception if all connections fail" do
        subject.should_receive(:send_command_to_server).with(:command, :primary_server) { raise BackgroundQueue::ConnectionError, "Primary" }
        subject.should_receive(:send_command_to_server).with(:command, :failover_1) { raise BackgroundQueue::ConnectionError, "Failure 1" }
        subject.should_receive(:send_command_to_server).with(:command, :failover_2) { raise BackgroundQueue::ConnectionError, "Failure 2" }
